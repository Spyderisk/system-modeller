...
public class ConstructionPattern extends Pattern {
	...
	private final Set<Node> inferredNodes;
	private final Set<Link> inferredLinks;
	//TODO: inferred node settings per inferred node

	public ConstructionPattern() {
		super();
		inferredNodes = new HashSet<>();
		inferredLinks = new HashSet<>();
	}
	...
	public String toString() {
		...
		s += "\n\t\tInferred Nodes: ";
		s = inferredNodes.stream().map(n -> n.toString()).reduce(s, String::concat);
		s += "\n\t\tLinks: ";
		s = links.stream().map(l -> l.toString()).reduce(s, String::concat);
		s += "\n\t\tInferred Links: ";
		s = inferredLinks.stream().map(l -> l.toString()).reduce(s, String::concat);
		return s;
	}
	...
	public Set<Node> getInferredNodes() {
		return inferredNodes;
	}

	public Set<Link> getInferredLinks() {
