...
class IncomingRelationsPanel extends React.Component {
    ...
    render() {
        const self = this;
        const incomingRelations = this.props.model["relations"].filter((relation) => relation["toID"] === self.props.selectedAsset["id"]);
        //console.log("incomingRelations", incomingRelations);

        let hiddenDiv;

        return (
            <div className="incoming-rel detail-list">
                <div className="container-fluid">
                    <span>{incomingRelations.length === 0 ? "No incoming relations defined" : ""}</span>
                    {incomingRelations.sort((relA, relB) => {
                        ...
                        let fromAssetInfo = fromAsset === undefined ?
                            "?"
                            :
                            fromAsset["label"];
                        ...
                        return (
                            <div key={"rel" + index} className="row detail-info rel-info row-hover"
                                ...
                                <OverlayTrigger delayShow={Constants.TOOLTIP_DELAY} placement="top" overlay={
                                    <Tooltip id={`inc-rel-${index}-tooltip`}
                                             className={"tooltip-overlay"}>
                                ...
                                <span className={"rel col-xs-10" + (relation["asserted"] ? " col-xs-offset-1" : "")}>
                                    <span style={{cursor: "pointer"}} className={"clickable"}
                                          onClick={() => this.props.dispatch(changeSelectedAsset(fromAsset.id))}>
                                        <strong>{fromAssetInfo}</strong>
                                    </span>
                                    {" "}
                                    <strong>
                        ...
                        <div className="define-rel">
                            <Button bsStyle="primary"
                                    ...
                                    onClick={() => this.props.handleAdd(self.props.model.assets
                                        .find((asset) => asset["id"] === self.props.selectedAsset["id"]), true)}>
                                {"+ Add"}
                            </Button>
                        </div>
                        :
...
IncomingRelationsPanel.propTypes = {
...
export default IncomingRelationsPanel;
